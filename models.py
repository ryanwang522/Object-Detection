...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.sigmoid(x)
        import math
        grid_size = int(math.sqrt(self.output_size // 26))
        x = x.view(-1,grid_size, grid_size, 26)
        return x
...
def Improved_model(pretrained=True, **kwargs):
    yolo = VGG(make_layers(cfg['D'], batch_norm=True), output_size=1664, **kwargs)
    if pretrained:
        vgg_state_dict = model_zoo.load_url(model_urls['vgg16_bn'])
        yolo_state_dict = yolo.state_dict()
        for k in vgg_state_dict.keys():
